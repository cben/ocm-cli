...
func run(cmd *cobra.Command, argv []string) error {
	...
	if args.version != "" {
		versionList, defaultVersion, err := provider.GetEnabledVersions(cmv1Client)
		if err != nil {
			return fmt.Errorf("unable to retrieve versions: %s", err)
		}

		clusterVersion = provider.DropOpenshiftVPrefix(args.version)
		...
		clusterVersion = provider.EnsureOpenshiftVPrefix(clusterVersion)
	}
